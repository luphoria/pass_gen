...
const key = prompt(
  "Enter XOR key to read passwords (YOU MUST REMEMBER THIS YOURSELF): "
);

let passwords = objAlpha(JSON.parse(file.readFileSync("passwords.json")));

while (true) {
  chooseAction = ""; // whatever action the user wants to perform
  while (
    ...
    chooseAction = prompt(
      "would you like to view your saved passwords (v), generate a new password (g), store an existing password/token (s), delete a password (d), or (exit)?: "
    );
    if (chooseAction.toLowerCase() === "exit") {
      console.log("bye!");
      process.exit(1);
    }
    if (
